...
async function getUserStats() {
  try {
    ...
    recentUsers.forEach((user: any, i: number) => {
      ...
      console.log(`      Plan: ${user.subscription_plan || 'free'} | Status: ${user.subscription_status || 'unknown'}`);
      console.log(`      AI: ${user.ai_messages_used || 0} | Calls: ${user.voice_calls_used || 0}`);
      console.log(`      Registriert: ${user.created_at ? new Date(user.created_at).toLocaleDateString('de-DE') : 'unknown'}`);
    });
