...
export function ChatInterface() {
  ...
  return (
    <div className="flex flex-col h-full bg-gradient-to-b from-black to-black/50 relative overflow-hidden" onDrop={handleDrop} onDragOver={handleDragOver} onDragLeave={handleDragLeave}>
      {isDragging && (
      ...
      <AnimatePresence>
        {showHistory && (
          <>
            ...
            <motion.div initial={{ x: -320 }} animate={{ x: 0 }} exit={{ x: -320 }} transition={{ type: "spring", damping: 25, stiffness: 200 }} className="fixed left-0 top-0 bottom-0 w-80 bg-black/95 backdrop-blur-xl border-r border-white/10 z-50 flex flex-col">
              <div className="p-4 border-b border-white/10 flex justify-between items-center">
                <div className="flex items-center space-x-2">
                  <MessageSquare className="w-5 h-5 text-[#FE9100]" />
                  <h3 className="text-white font-semibold">Chat-Historie</h3>
                </div>
                <Button size="sm" variant="ghost" onClick={() => setShowHistory(false)} className="text-gray-400 hover:text-white h-8 w-8 p-0">
                  <X className="w-4 h-4" />
                </Button>
              </div>
              <div className="flex-1 overflow-y-auto p-3 space-y-2 premium-scroll">
                {chatSessions.length > 0 ? (
                  chatSessions.map((session) => (
                    <motion.div key={session.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} whileHover={{ x: 4 }} onClick={() => loadChatSession(session.id)} className={`group p-3 rounded-xl cursor-pointer transition-all ${session.isActive ? "bg-[#FE9100]/10 border border-[#FE9100]/30" : "bg-white/5 hover:bg-white/10 border border-white/10"}`}>
                      <div className="flex items-start justify-between mb-2">
                        ...
                        {session.isActive && <div className="w-2 h-2 bg-[#FE9100] rounded-full animate-pulse" />}
                      </div>
                      <div className="flex items-center justify-between text-xs text-gray-400">
                        <div className="flex items-center space-x-1">
                          <Clock className="w-3 h-3" />
                          <span>{new Date(session.updatedAt).toLocaleDateString('de-DE', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })}</span>
                        </div>
                      </div>
                    </motion.div>
                  ))
              ...
              <div className="p-3 border-t border-white/10">
                <Button onClick={() => { startNewChatMutation.mutate(); setShowHistory(false); }} className="w-full bg-gradient-to-r from-[#FE9100] to-[#a34e00] hover:from-[#ff9d1a] hover:to-[#b55a00] text-white">
      ...
      <div className="p-2 border-b border-white/20 flex justify-between items-center">
        <Button size="sm" variant="ghost" onClick={() => setShowHistory(!showHistory)} className="text-gray-400 hover:text-white">
          <Menu className="w-4 h-4" />
        </Button>
        <div className="flex items-center space-x-2">
          <Button size="sm" variant="ghost" onClick={() => startNewChatMutation.mutate()} className="text-gray-400 hover:text-white">
      ...
      {/* Messages Area */}
      <div ref={messagesContainerRef} className="flex-1 overflow-y-auto p-6 space-y-4 relative z-10 premium-scroll">
        {allMessages.length === 0 ? (
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center">
              <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 0.5 }} className="mb-8">
                <img src={arasLogo} alt="ARAS AI" className="w-20 h-20 object-contain mx-auto" />
              </motion.div>
                ...
                <div className="flex items-center space-x-2">
                  <div className="flex space-x-1">
                    {[0, 0.2, 0.4].map((delay, i) => (
                      <motion.div key={i} className="w-2 h-2 bg-[#FE9100] rounded-full" animate={{ scale: [1, 1.5, 1], opacity: [1, 0.5, 1] }} transition={{ duration: 1, repeat: Infinity, delay }} />
                    ))}
                  </div>
        ...
        <div className="relative flex items-end space-x-3 max-w-4xl mx-auto">
          <div className="flex-1">
            <div className="relative">
              ...
              <Input value={message} onChange={(e) => setMessage(e.target.value)} onKeyPress={handleKeyPress} placeholder="Message ARAS AI" className="relative h-[42px] bg-black/80 backdrop-blur-sm text-white placeholder:text-gray-500 border-0 rounded-2xl focus:ring-1 focus:ring-[#FE9100]/20 transition-all duration-200 pr-16" disabled={sendMessage.isPending} />
              
              <Button onClick={isRecording ? stopRecording : startRecording} variant="ghost" size="sm" className="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-orange-600/20 hover:bg-orange-600/40 border border-orange-500/40 p-0 flex items-center justify-center transition-all duration-200" disabled={sendMessage.isPending}>
                {isRecording ? (
          ...
          <div className="flex items-center space-x-2">
            <Button onClick={() => fileInputRef.current?.click()} variant="outline" size="sm" className="h-[42px] px-3 border-white/30 hover:border-[#FE9100]/50 bg-transparent text-white">
              <Paperclip className="w-4 h-4" />
            </Button>
            <Button onClick={handleSendMessage} size="sm" disabled={!message.trim() || sendMessage.isPending} className="h-[42px] px-4 bg-gradient-to-r from-[#FE9100] to-[#a34e00] hover:from-[#ff9d1a] hover:to-[#b55a00] text-white border-0 rounded-xl transition-all duration-200">
